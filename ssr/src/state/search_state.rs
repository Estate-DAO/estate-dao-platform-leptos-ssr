use leptos::RwSignal;
use leptos::*;
use crate::component::{SelectedDateRange, GuestSelection};
...
impl SearchCtx{
    ...
    pub fn set_adults(num_adult: u32){
        ...
        this.guests.update(|guest_selection| guest_selection.adults.set(num_adult));
    }
    
    pub fn set_children(num_children: u32){
        let this: Self = expect_context();

        this.guests.update(|guest_selection| guest_selection.children.set(num_children));
    }
    ...
    pub fn set_children_ages(ages: Vec<u32>){
        ...
        if children_count >= 1 && ages.len() <= children_count as usize {
            this.guests.update(|guest_selection| guest_selection.children_ages.update(|existing_ages| existing_ages.extend(ages)));
        }
    }
