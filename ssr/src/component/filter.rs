            ...
            <div class="fixed inset-0 flex z-10">
                ...
                <div class="w-1/2 bg-white shadow-lg flex flex-col">
                    ...
                    <div class="flex space-x-40 p-6 px-16">
                        { ["PriceRange", "Amenities", "PropertyType", "Ratings"].iter().map(|filter| {
                            let filter_name = filter.clone();
                            ...
                            view! {
                                <button
                                    ...
                                    on:click=move |_| set_filter(filter_name.to_string())
                                >
                                    {filter_name}
                                    <span class="absolute left-0 right-0 bottom-[-.5rem] h-0.5 bg-black" style=if is_selected() { "display: block;" } else { "display: none;" }></span>
                                </button>
                            }
                        }).collect::<Vec<_>>() }
                    </div>
                    ...
                    <div class="p-12 flex justify-start space-x-4">
                        <button class="border border-blue-500 text-black px-4 py-2 rounded-full text-blue-500" on:click=move |_| set_is_open.set(false)>Clear All</button>
                        <button class="bg-blue-600 text-white px-4 py-2 rounded-full">Apply Filters</button>
                    </div>
                </div>
            </div>
        </Show>
...
fn PropertyType() -> impl IntoView {
    ...
    let is_selected = move |property_type: &str| {
        selected_type.get().as_deref() == Some(property_type)
    };
    view! {
        <div class="flex px-12">
